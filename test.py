...
def main(args):
    """Main function to data poisoning
    input:
    ...
    target_img_idx = args.target_img_idx
    for target_img_idx in target_img_idx:
        ...
        target_img, target_label = trainset[target_img_idx]
        # get multiple bases
        ...
        base_tensor_list = [bt.to(device) for bt in base_tensor_list]
    ...
    #   Save Poisons
        ...
        with open(os.path.join(args.poisons_path,str(target_img_idx), "poisons.pickle" ), "wb") as handle:
            pickle.dump(poison_tuple_list, handle, protocol=pickle.HIGHEST_PROTOCOL)
        with open(
            os.path.join(args.poisons_path, str(target_img_idx), "perturbation_norms.pickle"), "wb"
        ) as handle:
            pickle.dump(poison_perturbation_norms, handle, protocol=pickle.HIGHEST_PROTOCOL)
        with open(os.path.join(args.poisons_path,  str(target_img_idx),"base_indices.pickle"), "wb") as handle:
            pickle.dump(base_indices, handle, protocol=pickle.HIGHEST_PROTOCOL)
        with open(os.path.join(args.poisons_path, str(target_img_idx),"target.pickle"), "wb") as handle:
            pickle.dump(
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--model_path",
        default=["check_default/ResNet18_cifar10_1.pth"],
        nargs="+",
        type=str,
    )
    ...
    parser.add_argument(
        "--poisons_path",
        default="poison_examples/cp_poisons2",
        type=str,
    ...
    parser.add_argument(
        "--crafting_iters", default=600, type=int, help="iterations for making poison"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--target_img_idx",
        default=[46043, 34447, 18826, 745, 10113, 2997,6473,35349,21937,4450,30944,21177,
 25935,45794,16106,4904,16639,40128, 9681,18895,15023,4035, 38151,2713,36060],
        type=list,
        help="Index of the target image in the claen set.",
    )
    parser.add_argument(
        "--base_indices", nargs="+", default=[3823, 20104, 27464, 33244, 31894, 15513, 44329, 25340, 32707, 20487, 44068,9736,
 18621, 41668, 47479, 33543,3223,  8514,248, 15651, 40425, 15902 ,10431, 36702,
  1176], type=list, help="which base images"
    )
