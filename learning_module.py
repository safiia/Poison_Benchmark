...
def to_results_table(stats, out_dir, log_name="results11.csv"):
...
def train(net, trainloader, optimizer, criterion, device, train_bn=True):
    ...
    # poisons_correct = 0
    # poisons_seen = 0
    # for batch_idx, (inputs, targets, p) in tqdm(enumerate(trainloader)):
...
def load_model_from_checkpoint(model, model_path, dataset="CIFAR10"):
    ...
    state_dict = torch.load("check_default/ResNet18_cifar10_1.pth",map_location=device)#(model_path, map_location=device)
